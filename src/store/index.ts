...
export const createStore: typeof createStoreDecl = (): Store => {
  ...
  const callIdsAndHasListenersIfChanged = (
    ...
    if (!collIsEmpty(changedIds)) {
      callListeners(isInternal === 1, idListeners, ids, () =>
        mapToObj(changedIds),
      );
  ...
  const fluentTransaction = (
    ...
    internalTransaction(() => actions(...arrayMap(args, id)));
    return store;
  };
  ...
  const internalTransaction = <Return>(
    actions: () => Return,
    doRollback?: DoRollback,
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore void return only occurs internally
    ...
    localTransactions++;
    const result = transaction(actions, doRollback);
    localTransactions--;
    return result;
  };
  ...
  const callInternalListeners = () => {
    if (isRollingBack || localTransactions > 1) {
      return;
    }
  ...
  const finishTransaction = (doRollback?: DoRollback): Store => {
    if (transactions > 0) {
      ...
      // Process internal listeners except for deep internal transactions
      callInternalListeners();
